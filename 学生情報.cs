...
namespace HL_塾管理
{
    public partial class 学生情報 : DockContent
    {
        ...
        public bool flag_研究社員 = false;
        public string id_応募者 = null;
        public string id_社員 = null;
        public bool isfillend = false;
        public int iskensyu = 0;
        ...
        public void GetDataSource()
        {
            ...
            try
            {
                ...
                if (dtInfo.Rows.Count == 1)
                {
                    ...
                    string x = dtInfo.Rows[0]["研修フラグ"].ToString();

                    if (dtInfo.Rows[0]["研修フラグ"].ToString() == "False")
                    {
                        iskensyu = 0;
                        chb_研修社員.Enabled = false;
                        cmb_研修社員.Enabled = false;
                    }
                    else
                    {
                        iskensyu = 1;
                        chb_研修社員.Enabled = false;
                        chb_研修社員.Checked = true;
                        cmb_研修社員.Text= dtInfo.Rows[0].Field<string>("社員コード")+ " "+txt_名前.Text;
                        cmb_研修社員.Enabled = false;
                        ...
                        cmb_国籍.Enabled = false;
                        lbl_国籍.Text = lbl_国籍.Text.Replace("[必須]", "");
                        txt_学校名.Enabled = string.IsNullOrWhiteSpace(txt_学校名.Text);
                        if (!txt_学校名.Enabled)
                        {
                            lbl_学校名.Text = lbl_学校名.Text.Replace("[必須]", "");
                        }
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            {
                iskensyu = 1;
                id_社員 = cmb_研修社員.SelectedValue.ToString();
            }
        ...
        private void btn_クラス選択_Click(object sender, EventArgs e)
        {
            ...
            m_NewForm_クラス選択.研修フラグ = chb_研修社員.Checked.ToString();
        ...
        private void chb_研修社員_CheckedChanged(object sender, EventArgs e)
        {
            if (chb_研修社員.Checked && isUpdate == "new")
            {
                cmb_研修社員.Enabled = true;
                dtInfo = new DataTable();
                dtInfo.Columns.Add("社員コード");
                dtInfo.Columns.Add("名前");
                dtInfo.Rows.Add("-", "-");
                ...
                string str_sqlcmd = @"select t1.社員コード,t1.名前 from HL_JINJI_社員マスタ t1
                                      left join HL_JINJI_社員在職状態 t2
                                      on t1.社員コード = t2.社員コード
                                      right join HL_JINJI_応募者情報 t3
                                      on t2.社員コード = t3.社員コード
                                      where t2.社員種別 = '研修' and t2.在職状態 = '在職' and t1.社員コード  not in (select 社員コード from HL_JUKUKANRI_学生情報 where 社員コード is not null )";
                try
                {
                    SqlDataAdapter sqlDa = new SqlDataAdapter(str_sqlcmd, sqlcon);
                    sqlDa.Fill(dtInfo);
                    int rownum = dtInfo.Rows.Count;
                    for (int i = 0; i < rownum; i++)
                    {
                        ...
                        {
                            dtInfo.Rows[i][1] = string.Format("{0}　　 {1}", dtInfo.Rows[i][0].ToString(), dtInfo.Rows[i][1].ToString());
                        }
                    }


                    isfillend = false;
                    cmb_研修社員.DataSource = dtInfo;
                    cmb_研修社員.DisplayMember = "名前";
                    cmb_研修社員.ValueMember = "社員コード";
                    isfillend = true;
                ...
                {
                    this.toolStripStatusLabel1.ForeColor = Color.Red;
                    this.toolStripStatusLabel1.Text = "研修社員情報の獲得が失敗した。" + ex.Message;
                }
                finally
                {
                    if (sqlcon != null)
            ...
            else if (!chb_研修社員.Checked && isUpdate == "new")
            {
                cmb_研修社員.Enabled = false;
                dtInfo.Clear();
                cmb_研修社員.DataSource = dtInfo;
                ...
                txt_名前.Enabled = true;
                lbl_名前.Text = "名　前　[必須]";
                txt_ローマ字表記.Enabled = true;
                lbl_ローマ字表記.Text = "ローマ字表記　[必須]";
                txt_カタカナ.Enabled = true;
                lbl_カタカナ.Text = "カタカナ　[必須]";
                dtp_生年月日.Enabled = true;
                lbl_生年月日.Text = "生年月日　[必須]";
                cmb_性別.Enabled = true;
                lbl_性別.Text = "性　別　[必須]";
                txt_MyNumber.Enabled = true;
                lbl_MyNumber.Text = "MyNumber　[必須]";
                txt_郵便番号1.Enabled = true;
                txt_郵便番号2.Enabled = true;
                lbl_郵便番号.Text = "郵便番号　[必須]";
                txt_住所.Enabled = true;
                lbl_住所.Text = "住　所　[必須]";
                txt_携帯.Enabled = true;
                lbl_携帯.Text = "携　帯　[必須]";
                txt_メール.Enabled = true;
                lbl_メール.Text = "メール　[必須]";
                cmb_国籍.Enabled = true;
                lbl_国籍.Text = "国　籍　[必須]";
        ...
        private void cmb_研修社員_SelectedValueChanged(object sender, EventArgs e)
        {
            if (isfillend == false)
            {
                return;
            ...
            if (chb_研修社員.Checked && isUpdate == "new")
            {
                ...
                {
                    string str_sqlcmd = @"select t1.社員コード,
                                  　     t1.名前,
                                         t1.カタカナ,
                                         t1.ローマ字表記,
                                         t1.生年月日,
                                         ...
                                         t5.学校名 as 社員学校,
                                         t3.id as '応募者id' 
                                         from HL_JINJI_社員マスタ t1
                                         left join HL_JINJI_社員在職状態 t2
                                         on t1.社員コード=t2.社員コード
                                         right join HL_JINJI_応募者情報 t3
                                         on t1.社員コード=t3.社員コード
                                         left join HL_JINJI_社員情報 t4
                                         on t1.社員コード=t4.社員コード
                                         left join HL_JINJI_社員卒業学校 t5
                                         on t1.社員コード=t5.社員コード
                                         where t2.社員種別='研修' and t2.在職状態='在職'";
                    str_sqlcmd += string.Format("and t1.社員コード='{0}'", cmb_研修社員.SelectedValue);
                    try
                    {
                        SqlDataAdapter sqlDa = new SqlDataAdapter(str_sqlcmd, sqlcon);
                        sqlDa.Fill(dt);
                        ...
                        dtp_生年月日.Value = Convert.ToDateTime(dt.Rows[0]["生年月日"].ToString());
                        cmb_性別.Text = dt.Rows[0]["性別"].ToString();
                        txt_MyNumber.Text = dt.Rows[0]["MYNumber"].ToString();
                        code_郵便 = dt.Rows[0]["郵便番号"].ToString();
                        if (code_郵便.IndexOf("〒") >= 0)
                        {
                            code_郵便 = code_郵便.Replace("〒", "");
                        }
                        ...
                        txt_学校名.Text = dt.Rows[0]["社員学校"].ToString() == "" ? dt.Rows[0]["応募者学校"].ToString() : dt.Rows[0]["社員学校"].ToString();
                        lbl_応募者id.Text= dt.Rows[0]["応募者id"].ToString();
