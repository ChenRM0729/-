    ...
    public partial class クラス管理 : DockContent
    {
        ...
        private void Page_load(object sender, EventArgs e)
        {
            ...
            if (isUpdate == "new")
            {
                if (usercheck == "user")
                {
                    ...
                    lbl_教師コード.Text = lbl_教師コード.Text.Replace("[必須]", "");
                }
        ...
        public void SetClassInfo()
        {
            ...
            if (cmb_教師コード.Items.Count == 0)
            {
                ...
                {
                    str_sqlcmd = string.Format(@"SELECT 教師コード,名前 FROM HL_JUKUKANRI_教師情報 WHERE 教師コード = '{0}'", ((Form1)(Tag)).m_ユーザ登録.m_ログイン_教師コード);
                }
                else
                {
                    str_sqlcmd= "SELECT 教師コード,名前 FROM HL_JUKUKANRI_教師情報 ORDER BY 教師コード";
                }
                ...
                cmb_教師コード.DisplayMember = "教師コード";
                cmb_教師コード.ValueMember = "名前";
            ...
            else
            {
                ...
                else
                {
                    ...
                    cmb_教師コード.Text = dt.Rows[0]["教師コード"].ToString();
                    //code_教師コード = txt_教師名前.Text;
                    txt_教師名前.Text = dt.Rows[0]["名前"].ToString();
                ...
                for (int i = 0; i < dt1.Rows.Count; i++)
                {
                        ...
                        dtp_開始日.Value <= dts1 && dtp_終了日.Value >= dts2)
                    {
                        message = string.Format("教師[{0}]が[{1}～{2}]の期間内にクラス[{3}({4})]があるため登録できません。教師 または 日付を変更してください。"
                                , txt_教師名前.Text, dt1.Rows[i]["開始日"].ToString(), dt1.Rows[i]["終了日"].ToString(), dt1.Rows[i]["クラス名"].ToString(), dt1.Rows[i]["クラスコード"].ToString());
                        break;
                    }
                }
        ...
        public bool InputCheck()
        {
            ...
            {
                AddErrMsgList(lbl_教師コード.Text, "教師コードが入力されていません。");
                return false;
            }
            else
            {
        ...
        private bool IsExsitedWith教師コード(string code_教師コード)
        {
            ...
            {
                AddErrMsgList(lbl_教師コード.Text, "[教師コード]在チェック処理]:DBサーバーの接続に失敗しました。");
                return false;
            }
            ...
            if (!reader.Read())
            {
                AddErrMsgList(lbl_教師コード.Text, string.Format("[教師コード]({0})が存在していません。", code_教師コード));
                result = false;
            }
            else
            {
                if (code_教師コード.Substring(0, 1) != "K")
                {
                    ...
                    {
                        AddErrMsgList(lbl_教師コード.Text, string.Format("[教師コード]({0})が待機状態ではないため、登録できません。[社員から教師へ変更]を参照してください。", code_教師コード));
                        result = false;
                    }
                }
            }
        ...
        private void InsertData()
        {
            ...
            try
            {
                ...
                code_教師コード = cmb_教師コード.Text == null ? null : cmb_教師コード.Text;
                ...
                if (result == 1)
                {
                    sqlcom.CommandText = string.Format(@"Update HL_JUKUKANRI_教師情報 Set クラスコード = {0} Where 教師コード = '{1}'", txt_クラスコード.Text, code_教師コード);
                    result = sqlcom.ExecuteNonQuery();
            ...
            foreach (string controlName in errmsg.Keys)
            {
                switch (controlName)
                {
                    case "教室コード":
                        ...
                        break;
                    case "教師コード":
                        txt_教師名前.BackColor = System.Drawing.Color.Red;
                        break;
                    case "言語":
        ...
        private void Cmb_教師コード_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                txt_教師名前.Text = cmb_教師コード.SelectedValue.ToString();
                ...
                //社外教師の場合
                if (cmb_教師コード.Text.Length > 0 && cmb_教師コード.Text.Substring(0, 1) == "K")
                {
                    str_sqlcmd = string.Format(@"SELECT　名前, メイン言語 FROM HL_JUKUKANRI_社外教師マスタ WHERE 教師コード = '{0}'", cmb_教師コード.Text);
                }
                else
                {
