    ...
    public partial class 出勤表エラー記録 : DockContent
    {
        ...
        private void DisplayGridView()
        {
            ...
            string str_sqlcmd = "select ccc.出勤機コード,bbb.学生コード,ddd.名前,aaa.登録コード,ccc.出退勤フラグ,ccc.出退勤時間 from HL_JINJI_出勤機ダウンロード記録 aaa"
                             ...
                             + " where CONVERT(VARCHAR(6), ccc.出退勤時間, 112) = '" + date_日付 + "' and ";
            ...
            str_sqlcmd += "group by ccc.出勤機コード,bbb.学生コード,ddd.名前,aaa.登録コード,ccc.出退勤フラグ,ccc.出退勤時間 order by bbb.学生コード,ccc.出退勤時間";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    roomView.Rows[rowIndex].Cells["出退勤時間"].Value = DateTime.Parse(dt.Rows[i]["出退勤時間"].ToString()).ToString("yyyy/MM/dd HH:mm:ss");
