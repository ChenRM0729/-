...
namespace HL_塾管理
{
    public partial class 学生情報一覧 : DockContent
    {
        ...
        private void DisplayGridView()
        {
            ...
            if(職務 == "一般ユーザ")
            {
                str_sqlcmd += string.Format(" Where T3.クラスコード = '' or T3.クラスコード is null or T4.教師コード = '{0}'", login_教師コード);
            }
                ...
                while (reader.Read())
                {
                    ...
                    foreach (DataGridViewCell gvCell in this.gv_studentsInfo.Rows[index].Cells)
                    {
                        if (string.IsNullOrWhiteSpace(gvCell.Value.ToString()) || gvCell.Value.ToString() == "-")
                        {
                            ...
                            //chen 20200720 add
                            //研修社員の場合は編集不可
                        if (this.gv_studentsInfo.Rows[index].Cells["研修"].Value.ToString() == "True")
                        {
        ...
        /// 学生評価画面へ遷移する
        /// </summary>
        private void 学生評価ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            code_学生コード = gv_studentsInfo.CurrentRow.Cells["学生コード"].Value.ToString();
            ...
            m_NewForm_学生評価.code_クラス = code_クラスコード;
            m_NewForm_学生評価.flag_学生情報 = true;
            m_NewForm_学生評価.Tag = ((Form1)(this.Tag));
            m_NewForm_学生評価.Show(((Form1)(this.Tag)).dockPanel1);
            ((Form1)(this.Tag)).m_学生評価Handle = m_NewForm_学生評価.Handle;

            int ptr = (int)((Form1)(this.Tag)).m_学生評価Handle;
            if (!((Form1)(this.Tag)).codeDic.ContainsKey(code_学生コード))
            {
                ((Form1)(this.Tag)).codeDic.Add(code_学生コード, ptr);
            }
        ...
        private void クラス課程選択ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            m_NewForm_クラス選択.研修フラグ = this.gv_studentsInfo.CurrentRow.Cells["研修"].Value.ToString();
