...
　　　　　　　　　　　　　　　　　　　　　　　　LEFT JOIN HL_JUKUKANRI_教室マスタ d ON a.教室コード = d.教室コード
                                                WHERE a.有効 = 1 ORDER BY a.クラスコード ");
        ...
        private void 学生評価ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //クラス管理呼び出す
            学生評価 m_学生評価 = new 学生評価();

            m_学生評価.code_クラス = rowMergeView1.CurrentRow.Cells["クラスコード"].Value.ToString();
            m_学生評価.flag_クラス情報 = true;
            m_学生評価.Tag = ((Form1)(Tag));
            m_学生評価.Show(((Form1)(Tag)).dockPanel1);
            ((Form1)(Tag)).m_学生評価Handle = m_学生評価.Handle;

            int ptr = (int)((Form1)(Tag)).m_学生評価Handle;
            //曜日出した画面が複数画面開かないチェックのため、教室コード入れる。
            if (!((Form1)(Tag)).codeDic.ContainsKey(codeClass))
            {
                ((Form1)(Tag)).codeDic.Add(codeClass, ptr);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void 学生選択ToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            ((Form1)(Tag)).toolStripStatusLabel2.Text = "";
